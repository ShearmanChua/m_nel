...
def entitySearch(query):
    ...
    for result in elasticResults['hits']['hits']:
        if result["_source"]["label"].lower().replace('.','').strip()==query.lower().strip():
            results.append([result["_source"]["label"],result["_source"]["id"],result["_score"]*50,40])
        else:
            results.append([result["_source"]["label"],result["_source"]["id"],result["_score"]*40,0])
    ...
    for result in elasticResults['hits']['hits']:
        ...
        if edit_distance<=1:
            results.append([result["_source"]["label"],result["_source"]["id"],result["_score"]*50,30])
        else:
            results.append([result["_source"]["label"],result["_source"]["id"],result["_score"]*25,0])
            
    results= sorted(results, key = lambda x: (int(x[1][x[1].rfind("/")+2:]),-x[3],-x[2]))
    ...
    results=results[:20]
    results= sorted(results, key = lambda x: (-x[3],int(x[1][x[1].rfind("/")+2:])))
