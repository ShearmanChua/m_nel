...
nlp = spacy.load('resources/spacy/en_core_web_sm')
...
def mix_list_items(mixedRelations,k):
    ...
    for raw in mixedRelations:
        if any(relation[3]>0 for relation in raw):
            for relation in sorted(raw, key=lambda x: (-x[3],int(x[1][x[1].rfind("/")+2:]),-x[2]))[:k]:
            # for relation in sorted(raw, key=lambda x: (-x[3],int(x[1][x[1].rfind("/")+2:-1]),-x[2]))[:k]:
                relations.append(relation)
        else:
            raw= sorted(raw, key = lambda x: (-x[2],int(x[1][x[1].rfind("/")+2:])))
...
def process_text_E_R(question,rules=[1,2,3,4,5,8,9,10,12,13,14],k=3):
    try:
        raw=evaluate([question],rules,evaluation=False, k=k)
    except Exception as e:
        print(f"\n  Error occured in Falcon2 for following Question: {question}")
        raise
    question=question.replace("?","")
    ...
    for token in doc:
        ...
        if  (not token.is_stop) and ( (token.dep_=="compound" and token.pos_!="PROPN") or token.pos_=="VERB" or token.dep_ == "ROOT"):
            ...
            for ent in entities: 
                ...
                if token.text in ent:
                    # print(token.text)
                    # ent_list=[token.text for token in nlp(ent) ]
                    ent_list=ent.split(' ')
                    next_token=doc[token.i+1]
                    # print(f"{ent_list} - {token.text}")
                    if ent_list.index(token.text)!= len(ent_list)-1 and next_token.dep_ =="compound":
                        isEntity=True
                        break
            if not isEntity:
...
def evaluate(raw,rules, k=1, evaluation=True):
    try:
...
if __name__ == '__main__':
    ...
    threading=False
    rules = [1,2,3,4,5,8,9,10,12,13,14]
    print( process_text_E_R("The US left Agfghanistan.", rules, k=3) ) 
    # print( process_text_E_R("Luke Air Force Base Introduction /n", rules, k=3) ) # causes error
    # print( process_text_E_R("In Asia-Pacific", rules, k=3) )
    # print( process_text_E_R("Asia-Pacific", rules, k=3) ) # causes error
    # print( process_text_E_R("What countries are in Asia-Pacific", rules, k=3) )
    # print( process_text_E_R("Hong Kong's", rules, k=3) )
    # print( process_text_E_R('Beijing',rules, k=3) )
    # print( process_text_E_R('Earth',rules, k=3) )
    # print( process_text_E_R('Who is the wife of barack obama?',rules, k=3) )
    # print( process_text_E_R('Michelle is the wife of barack obama?',rules, k=3) )
    # print( process_text_E_R('Michelle Obama',rules, k=3) )
